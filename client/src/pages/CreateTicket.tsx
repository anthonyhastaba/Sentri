...
const createTicketValidation = insertTicketSchema.extend({
  title: z.string().min(3, "Ticket title is required"),
  content: z.string().min(1, "Please describe the issue"),
});

type CreateTicketFields = z.infer<typeof createTicketValidation>;
...
export default function CreateTicket() {
  ...
  const form = useForm<CreateTicketFields>({
    resolver: zodResolver(createTicketValidation),
    mode: "onChange",
    defaultValues: {
      title: "",
      content: "",
  ...
  const isSubmitDisabled = isPending || (title ?? "").trim().length < 3;
      ...
      <main className="flex-1 md:ml-64 p-4 md:p-8 pt-14 md:pt-8">
        <div className="max-w-3xl mx-auto">
          ...
          <header className="mb-8">
            ...
            <p className="text-muted-foreground">Describe the incident — paste logs, error messages, or a user report. GPT-4o handles the triage.</p>
          </header>
