import { useTicket, useAnalyzeTicket, useUpdateTicket, useDeleteTicket } from "@/hooks/use-tickets";
import { Sidebar } from "@/components/Sidebar";
import { StatusBadge, PriorityBadge } from "@/components/StatusBadge";
import { useLocation, useRoute, Link } from "wouter";
import { ArrowLeft, Loader2, Sparkles, AlertTriangle, CheckCircle2, Copy, Trash2, BrainCircuit } from "lucide-react";
          ...
          <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
            <div>
              <div className="flex items-center gap-3 mb-2">
                <span className="font-mono text-xs text-muted-foreground bg-secondary px-2 py-1 rounded">ID: #{ticket.id}</span>
                <span className="text-xs text-muted-foreground">{ticket.createdAt && format(new Date(ticket.createdAt), "PPp")}</span>
              </div>
              <h1 className="text-2xl md:text-3xl font-bold tracking-tight">{ticket.title}</h1>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex flex-col items-end gap-1">
                ...
                <PriorityBadge priority={ticket.priority || "TBD"} className="h-9 px-3 text-sm" />
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="glass-panel p-6 rounded-xl border border-border/50">
                <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-3">Original Report</h3>
                ...
                <div className="glass-panel relative p-6 rounded-xl border border-primary/20">
                  ...
                  {!isAnalyzing && !ticket.category && (
                    <div className="text-center py-10 text-muted-foreground bg-secondary/20 rounded-lg border border-dashed border-border">
                      ...
                      <p>Run AI analysis to categorize, prioritize, and generate next steps.</p>
                    </div>
                  )}
                      ...
                      >
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div className="p-4 rounded-lg bg-secondary/30 border border-border/50">
                            <span className="text-xs uppercase text-muted-foreground font-semibold">Category</span>
                            <div className="text-lg font-medium mt-1 text-foreground">{ticket.category}</div>
                          </div>
                          <div className="p-4 rounded-lg bg-secondary/30 border border-border/50">
                            <span className="text-xs uppercase text-muted-foreground font-semibold">Recommended Priority</span>
  ...
  return (
    <div className="min-h-screen bg-background flex items-center justify-center">
      <div className="flex flex-col items-center gap-4">
        ...
        <p className="text-muted-foreground animate-pulse text-sm">Loading ticket details...</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="min-h-screen bg-background flex items-center justify-center p-4">
      <div className="glass-panel p-8 rounded-xl border border-destructive/20 text-center max-w-md">
        ...
        <p className="text-muted-foreground mb-6">The ticket you are looking for doesn't exist or you don't have permission to view it.</p>
        <Link href="/" className="inline-flex items-center justify-center px-4 py-2 bg-secondary hover:bg-secondary/80 rounded-md text-sm font-medium transition-colors">
          Return to Dashboard
        </Link>
      </div>
    </div>
