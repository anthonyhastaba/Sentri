...
import { ShieldAlert, LayoutDashboard, PlusCircle, Menu, X, BarChart2, Sparkles } from "lucide-react";
...
export function Sidebar() {
  ...
  const NavContent = ({ onNav, onPremium }: { onNav?: () => void; onPremium: () => void }) => (
    <>
      <div className="p-6 border-b border-border/50 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs uppercase font-bold text-muted-foreground tracking-[0.2em]">Security Operations</p>
        </div>
      </div>
      <nav aria-label="Main navigation" className="flex-1 p-4 space-y-1">
        <p className="px-4 py-2 text-xs font-semibold uppercase tracking-widest text-muted-foreground/70">Menu</p>
        {navItems.map((item) => {
          const isActive =
            location === item.href ||
            (item.href === "/" && location.startsWith("/tickets/") && location !== "/tickets/new");
          return (
            <Link key={item.href} href={item.href} onClick={onNav} className={cn(
              "flex items-center gap-3 px-4 py-3 rounded-md transition-all duration-200 group",
              isActive
                ? "border-l-2 border-primary text-primary bg-primary/10 rounded-none"
        ...
        <p className="px-4 py-2 text-xs font-semibold uppercase tracking-widest text-muted-foreground/70 pt-4">Insights</p>
        <button
          onClick={() => {
            onNav?.();
            onPremium();
          }}
          className="flex items-center gap-3 px-4 py-3 rounded-md transition-all duration-200 group text-muted-foreground hover:bg-secondary/50 hover:text-foreground w-full text-left"
        >
          <BarChart2 className="w-5 h-5 text-muted-foreground group-hover:text-foreground" />
  ...
  return (
    <>
      ...
      {mobileOpen && (
        <div className="md:hidden fixed inset-0 z-50 flex">
          ...
          <div className="relative w-64 bg-card border-r border-border flex flex-col h-full">
            ...
            <NavContent onNav={() => setMobileOpen(false)} onPremium={() => setPremiumOpen(true)} />
          </div>
        </div>
      )}
      ...
      <div className="w-64 border-r border-border bg-card/50 flex-col h-screen fixed left-0 top-0 hidden md:flex">
        <NavContent onPremium={() => setPremiumOpen(true)} />
